...
export default function OutreachSteps({campSteps, setCampSteps, campType}) {
  ...
  const handleShowAction1Content =  () => setShowAction1Content(!showAction1Content);
  ...
  const handleShowAction4Content =  () => {
    if (!action4ViewProfile && !action4Likes && !action4Follow && !action4Endorse) {
        removeStep(stepIndex);
    }
    setShowAction4Content(!showAction4Content);
  };
  const handleShowAction5Content =  () => setShowAction5Content(!showAction5Content);
  ...
  const handleShowAction7Content =  () => setShowAction7Content(!showAction7Content);
  ...
  function updateAction1(index) {
    ...
    updateStep[index].msg = action1Msg;
  ...
  function updateAction4(index) {
    if (!action4ViewProfile && !action4Follow && !action4Likes && !action4Endorse) {
  ...
  function updateAction8(index) {
    let urlExp = new RegExp(/^(ftp|http|https):\/\/[^ "]+$/);
    if (action8URL === '') {
        toast.error('Please provide URL!');
    }
    else if (!urlExp.test(action8URL)) {
  ...
  function handleEditAction(index) {
    setStepIndex(index);
    let item = campType === '1' ? campSteps[index] : campSteps.filter(item => item.name === 'Perform Action')[index];
    switch (item.name) {
        case 'Send Connection Request':
  ...
  const handleDragEnd = (droppedItem) => {
    ...
    var updatedList = [...campSteps];
    // Remove dragged item
    const [reorderedItem] = updatedList.splice(droppedItem.source.index, 1);
    // Add dropped item
    updatedList.splice(droppedItem.destination.index, 0, reorderedItem);
    // Update State
    setCampSteps(updatedList);
  };

  return (
    <div className='row g-0'>
        <div className='col-md-12'>
            <p className='mt-4 fw-600'>Outreach Steps</p>
            <div className='outreach-steps d-flex flex-wrap mt-4'>
                {
                    campSteps === 0 ?
                    '' :
                    campType === '2' ?
                    campSteps.filter(item => item.name === 'Perform Action').map((item, index) => {
                    ...
                    <DragDropContext onDragEnd={handleDragEnd}>
                        <Droppable droppableId='stepsList' direction='horizontal'>
                            {(provided) => (
                                <div
                                            ...
                                            return (
                                                ...
                                                >
                                                    {(provided) => (
                                                        ...
                                                        >
                                                            <div className='step-wrap d-flex align-items-center justify-content-center'>
                                                                <div className='remove-icon' onClick={() => removeStep(index)}>
                                                                    <BsX size={20} />
                                                                </div>
                                                                <div className='step box-card d-flex mb-3 p-2' onClick={() => handleEditAction(index)}>
                ...
                <div className='row g-0 py-3'>
                    <div className='col-md-6 pe-3 border-end'>
                        ...
                        <Button 
                            ...
                            onClick={addAction1}
                            disabled={campType === '2' ? true : false}
                            >
                            <div className='start'>
